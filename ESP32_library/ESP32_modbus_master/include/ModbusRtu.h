...
public:
    Modbus(uint8_t u8id, Stream& port, uint8_t u8txenpin =1);
    ...
    Modbus(uint8_t u8id=0, uint8_t u8serno=0, uint8_t u8txenpin=1) __attribute__((deprecated));
...
void Modbus::start()
{
    if (u8txenpin > 1)   // pin 0 & pin 1 are reserved for RX/TX
    {
        ...
        // digitalWrite(RE_PinNumber, LOW);
        // pinMode(DE_PinNumber, OUTPUT);
        // digitalWrite(DE_PinNumber, LOW);
    }
...
void Modbus::sendTxBuffer()
{
    ...
    if (u8txenpin > 1)
    {
        ...
        digitalWrite(RE_PinNumber, HIGH);
        digitalWrite(DE_PinNumber, HIGH);
    }
